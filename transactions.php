...
foreach ($transactions as $transaction) {
    if ($transaction['type'] === 'sent' && $transaction['sender'] === $userNumber) {
        $sentTransactions[] = $transaction;
    } elseif ($transaction['type'] === 'received' && $transaction['recipient'] === $userNumber) {
...
if (file_exists($history_file)) {
    ...
    if (isset($all_history[$user['number']])) {
        $history = $all_history[$user['number']];
    }
}
?>
<!DOCTYPE html>
...
<!--	<link rel="stylesheet" href="assets/css/sstyle.css"> -->
    ...
    <title>Transactions ~ Pavan Cash Loot</title>
...
<body class="bg-gray-100">
    ...
    <div id="sidebar" class="sidebar">
        ...
        <ul>
            ...
            <li style="background-color: blue;"><i class="fas fa-exchange-alt"><a href="transactions.php"></i>Transaction History</a></li>
...
<font color='green' style='font-weight:bold;'>Sent Transactions <i class='fas fa-exchange-alt' ></i></font><br>

<br>
            <center>
                    <div class="section">
        <h2 style="font-weight:bold; color: red;">Sent Transactions</h2>
        <?php if (empty($sentTransactions)): ?>
            <p style="font-weight:bold; color:brown;">No sent transactions found.</p>
        <?php else: ?>
            <div class="card-container">
                <?php foreach ($sentTransactions as $transaction): ?>
                    <div class="card sent" onclick="showTransactionDetails('<?php echo htmlspecialchars(json_encode($transaction)); ?>', 'sent')">
                        <span class="material-icons card-icon sent" style="color: DodgerBlue;">swap_horiz</span>
                        <div class="card-text" style="font-weight:bold; text-align: left;">
                            <h3 style="color: blue;"><spam style="color: red;">To:</spam> <?php echo htmlspecialchars($transaction['recipient']); ?></h3>
                            <p style="color: green;"><spam style="color: red;">Amount: </spam>₹<?php echo htmlspecialchars($transaction['amount']); ?></p>
                            <p><spam style="color: red;">Date:</spam> <?php echo date('d-m-Y h:i A', strtotime($transaction['date'] . ' ' . $transaction['time'] . ' +05:30')); ?></p>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php endif; ?>
			...
			<br><div style=' padding:10px; background: white; border-top:3px solid Blue;border-right:3px solid Blue;border-bottom:3px solid Blue;border-left:3px solid Blue; border-radius:10px;'>
<font color='green' style='font-weight:bold;'>Received Transactions <i class="fas fa-exchange-alt"></i></font><br>

<br>
            <center>
                    <div class="section">
        <h2 style="font-weight:bold; color: red;">Received Transactions</h2>
        <?php if (empty($receivedTransactions)): ?>
            <p style="font-weight:bold; color:brown;">No received transactions found.</p>
        <?php else: ?>
            <div class="card-container">
                <?php foreach ($receivedTransactions as $transaction): ?>
                    <div class="card received" onclick="showTransactionDetails('<?php echo htmlspecialchars(json_encode($transaction)); ?>', 'received')">
                        <span class="material-icons card-icon received" style="color: DodgerBlue;">swap_horiz</span>
                        <div class="card-text" style="font-weight:bold; text-align: left;">
                            <h3><spam style="color: red;">From:</spam> <?php echo htmlspecialchars($transaction['sender']); ?></h3>
                            <h3 style="color: green;"><spam style="color: red;">Amount: </spam>₹<?php echo htmlspecialchars($transaction['amount']); ?></h3>
                            <p><spam style="color: red;">Date:</spam> <?php echo date('d-m-Y h:i A', strtotime($transaction['date'] . ' ' . $transaction['time'] . ' +05:30')); ?></p>
                        </div>
                    </div>
                <?php endforeach; ?>
 ...
 <font color="blue" style="font-weight:600">
			<br><div style=' padding:10px; background: white; border-top:3px solid Blue;border-right:3px solid Blue;border-bottom:3px solid Blue;border-left:3px solid Blue; border-radius:10px;'>
<font color='green' style='font-weight:bold;'>Pay On Wallet To Any User Transactions <i class='fas fa-exchange-alt' ></i></font><br>

<br>
            <center>
                    <div class="section">
        <h2 style="font-weight:bold; color: red;">Pay On Wallet To Any User Transactions</h2>
        <?php if (empty($sentTransactions)): ?>
            <p style="font-weight:bold; color:brown;">No Pay On Wallet To Any User transactions found.</p>
        <?php else: ?>
            <div class="card-container">
                <?php foreach ($history as $entry): ?>
                    <div class="card sent" onclick="showTransactionDetails('<?php echo htmlspecialchars(json_encode($entry)); ?>', 'sent')">
                        <span class="material-icons card-icon sent" style="color: DodgerBlue;">swap_horiz</span>
                        <div class="card-text" style="font-weight:bold; text-align: left;">

                            <p style="color: green;"><spam style="color: red;">Type: </spam><?php echo htmlspecialchars($entry['type']); ?></p>
                            <p style="color: green;"><spam style="color: red;">Amount: </spam>₹<?php echo htmlspecialchars($entry['amount']); ?></p>
                            <p style="color: green;"><spam style="color: red;">PCL Wallet Number: </spam><?php echo htmlspecialchars($entry['target']); ?></p>
                            <p><spam style="color: red;">Date:</spam> <?php echo date('d-m-Y h:i A', strtotime($entry['date'] . ' ' . $entry['time'] . ' +05:30')); ?></p>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
...
<div class="w-full p-6">
  ...
  <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    ...
    <script>
        function showTransactionDetails(transactionData, type) {
            const transaction = JSON.parse(transactionData);
            const iconHtml = type === 'sent'
                ? `<span class="material-icons" style="color: #e74c3c; font-size: 40px;">check_circle</span>` // Red for sent
                : `<span class="material-icons" style="color: #2ecc71; font-size: 40px;">check_circle</span>`; // Green for received

            Swal.fire({
                title: `${type === 'sent' ? 'Sent' : 'Received'} Funds Details`,
                html: `
                    ${iconHtml}
                    <p><strong>${type === 'sent' ? 'Recipient' : 'Sender'}:</strong> ${transaction[type === 'sent' ? 'recipient' : 'sender']}</p>
                    <p><strong>Amount:</strong> ₹${transaction.amount}</p>
                    <p><strong>Comment:</strong> ${transaction.comment || 'No comment'}</p>
                    <p><strong>Date:</strong> ${new Date(transaction.date + ' ' + transaction.time + ' UTC').toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}</p>
                `,
                icon: type === 'sent' ? 'success' : 'success',
