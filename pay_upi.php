...
<!--	<link rel="stylesheet" href="assets/css/sstyle.css"> -->
    ...
    <title>Withdrawal UPI ~ Pavan Cash Loot</title>
...
<font color='brown' style='font-weight:bold;'>Withdrawal UPI ID <i class='bx bxs-wallet' ></i></font><br>

<br>
    <form method="POST">
       <label for="recipient_number" style="font-weight: bold; color:red;">Amount</label>
        <input type="number" name="amount" style="  padding: 10px;
  ...
  font-weight: bold;" placeholder="Enter Withdrawal Amount"  required>
       <br><br><label for="recipient_number" style="font-weight: bold; color:red;">Upi ID</label><br>
        <input type="text" name="upi_id" style="  padding: 10px;
  border: 2px solid #ddd;
  border-radius: 8px;
  width: 100%;
  margin-top: 8px;
  ...
  color: #333;
  transition: border 0.3s ease-in-out;
  font-weight: bold;" placeholder="Enter UPI ID"  required>
   <br><br>  <label for="recipient_number" style="font-weight: bold; color:red;">Comment</label><br>
        <input name="note" style="  padding: 10px;
  border: 2px solid #ddd;
  border-radius: 8px;
  width: 100%;
  margin-top: 8px;
  ...
  color: #333;
  transition: border 0.3s ease-in-out;
  font-weight: bold;" placeholder="Enter Comment" ></input>
     <br><br><center><button class="submit" style='font-weight:bold;'>Send </button></center>
    </form>
...
<font color='brown' style='font-weight:bold;'>Note Box <i class='bx bxs-note' ></i></font>
<center><br><div class="note-box">
   <p>Minimum: 50₹</p> 
   <p>Maximum: 500₹</p> 
   <p>Tax: 1₹</p> 
</div></center>
<br>
 ...
 <font color="blue" style="font-weight:600">
			<div style=' padding:10px; background: white; border-top:3px solid Blue;border-right:3px solid Blue;border-bottom:3px solid Blue;border-left:3px solid Blue; border-radius:10px;'>
<font color='brown' style='font-weight:bold;'>Withdrawal History <i class='bx bxs-history' ></i></font><br>

<br>
<h3 style='font-weight:bold;'>Your Withdrawal History</h3>
<div class="container">
    <?php
    $withdrawals = loadWithdrawals();
    foreach ($withdrawals as $withdrawal):
        if ($withdrawal['user_id'] === $user['number']): ?>
            <div 
                class="withdrawal-card <?= 'status-' . strtolower($withdrawal['status']) ?>" 
                onclick="showDetails('<?= $withdrawal['id'] ?>', '<?= $withdrawal['status'] ?>', '<?= $withdrawal['amount'] ?>', '<?= $withdrawal['upi_id'] ?>', '<?= $withdrawal['date'] ?>', '<?= addslashes($withdrawal['note']) ?>')"
            >
                ...
                <div class="card-action">
                    <span class="status-label"><?= $withdrawal['status'] ?></span>
                </div>
            </div>
        <?php endif;
    endforeach;
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        withdrawalForm.addEventListener("submit", function(event) {
            ...
            // Validation for minimum and maximum withdrawal limits
            ...
            // Confirmation for submitting the withdrawal
            Swal.fire({
                title: 'Are you sure?',
                text: `You are about to withdraw ₹${amount} to the UPI ID: ${upiId}.`,
                icon: 'question',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    // Make the AJAX call to submit the withdrawal
    ...
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11.7.5/dist/sweetalert2.all.min.js"></script>
<script>
function showDetails(id, status, amount, upiId, date, note) {
    const icon = status.toLowerCase() === 'approved' ? 'success' : 
                 status.toLowerCase() === 'rejected' ? 'error' : 'info';
