...
// Lifafa URL base
$baseURL = "https://PavanCashLoot.site/redeem.php";

// Handle Lifafa creation
$lifafaCreated = false;
$lifafaURL = "";
$error = "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $usersCount = intval($_POST['users'] ?? 0);
    $comment = $_POST['comment'] ?? '';
    $link = $_POST['link'] ?? '';
    $code = $_POST['code'] ?? '';

    // Validate inputs
    if (empty($title) || $amount <= 0 || $usersCount <= 0 || empty($comment) || empty($link) || empty($code)) {
        $error = "Please fill all fields correctly.";
    } elseif ($amount * $usersCount > $user['balance']) {
        $error = "Insufficient balance to create this Lifafa.";
    } else {
        // Create new Lifafa entry
        $newLifafa = [
            'title' => $title,
            'amount' => $amount,
            'users' => $usersCount,
        ...
        $user['balance'] -= $amount * $usersCount;
        ...
        // Generate Lifafa URL
        $lifafaURL = $baseURL . "?code=" . urlencode($code);
        $lifafaCreated = true;
    }
}
?>
...
<font color='red' style='font-weight:bold;'>Create Normal Lifafa <i class='bx bxs-gift' ></i></font><br><hr><br>
    ...
    <?php if ($lifafaCreated): ?>
        <p class="color-green w-full font-bold">Share this Lifafa URL with others:</p><br><br>
        <input type="text" id="lifafaURL" class="w-full p-3 border border-gray-300 rounded-md placeholder-gray-400 font-bold" value="<?php echo htmlspecialchars($lifafaURL); ?>" readonly><br><br>
        <button onclick="copyToClipboard()" class="submit logout-btn w-full font-bold">Copy URL</button>
    <?php else: ?>
        <form method="POST" action="">
            ...
            <input type="number" id="amount" name="amount" step="0.01" class="w-full p-3 border border-gray-300 rounded-md placeholder-gray-400 font-bold" placeholder="Enter Amount Per User" required><br><br>


            <input type="number" id="users" name="users" class="w-full p-3 border border-gray-300 rounded-md placeholder-gray-400 font-bold" placeholder="Enter Number Of Users" required><br><br>


            <textarea id="comment" name="comment" class="w-full p-3 border border-gray-300 rounded-md placeholder-gray-400 font-bold" placeholder="Enter Payment Comment" required></textarea><br><br>


            <input type="text" id="link" name="link" class="w-full p-3 border border-gray-300 rounded-md placeholder-gray-400 font-bold" placeholder="Enter Redirect Link" required><br><br>


            <input type="text" id="code" name="code" class="w-full p-3 border border-gray-300 rounded-md placeholder-gray-400 font-bold" placeholder="Enter Access Code" required><br><br>

            <button type="submit" class="submit logout-btn w-full font-bold">Create Lifafa</button>
        </form>
     <!--   <p class=" w-full font-bold">Your current balance: <?php echo number_format($user['balance'], 2); ?></p>-->
    <?php endif; ?><br>
